...
app.post('/exchange', async (req, res) => {
  try {
    ...
    // Сохраняем новый курс обмена в таблицу истории (фиксированный курс)
    ...
    if (rateInsertError) {
      ...
      // Продолжаем выполнение, если не критично
    }
    
    // Записываем операцию обмена в таблицу exchange_transactions с новым полем exchanged_amount
    const { error: insertError } = await supabase
      .from('exchange_transactions')
      .insert([{
        user_id: userId,
        direction,
        amount,
        exchanged_amount: exchangedAmount.toFixed(5),  // сохраняем exchanged_amount
        new_rub_balance: newRubBalance.toFixed(2),
        new_coin_balance: newCoinBalance.toFixed(5),
        created_at: new Date().toISOString(),
        exchange_rate: FIXED_RATE
      }]);
    if (insertError) {
    ...
    // Возвращаем обновленные балансы, курс и exchanged_amount
    return res.json({
      success: true,
