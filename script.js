...
// Функция для форматирования чисел с точностью до 0.00001
function formatBalance(balance) {
    return parseFloat(balance).toFixed(5); // Оставляем 5 знаков после запятой
}
...
function displayTransactionHistory(transactions) {
    ...
    transactions.forEach(tx => {
        ...
        const amount = formatBalance(tx.amount); // Форматируем сумму с точностью до 0.00001
...
// Открытие модального окна GugaCoin
